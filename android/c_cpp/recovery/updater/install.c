...
/* <--LH_SWRD_CL1_Mervins@2011.07.14:add mbr fuse check function. */
// Is16GInandFn(mount_point)
Value* Is16GInandFn(const char* name, State* state, int argc, Expr* argv[]) {

	int fd;
	char str[20];
	unsigned int size = 0;
	UpdaterInfo* ui = (UpdaterInfo*)(state->cookie);
	fd = fopen("/sys/devices/platform/mmci-omap-hs.1/mmc_host/mmc0/mmc0:0001/block/mmcblk0/size","r");
	if(fd==NULL)
	{
		fprintf(ui->cmd_pipe, "E: fd file open error!\n\n");
		return StringValue(NULL);
	}
	fgets(str, sizeof(str), fd);
	size = strtol(str,NULL,10);
    fprintf(ui->cmd_pipe, "ui_print size:%d\n", size);
	if (size > 30000000)
	{
		return StringValue(strdup("1"));
	}else if( size>0 && size<30000000 )
	{
...
/* LH_SWRD_CL1_Mervins@2011.07.14--> */
...
Value* RunProgramFn(const char* name, State* state, int argc, Expr* argv[]) {
    if (argc < 1) {
        ErrorAbort(state, "%s() expects at least 1 arg", name);
        return NULL;
    }
    char** args = ReadVarArgs(state, argc, argv);
    if (args == NULL) {
	...
	if (error_status == 0 ) {
		fprintf(stderr, "run_program success");
		return StringValue(strdup("success"));
	} else {
		fprintf(stderr, "run_program fail");
		return StringValue(strdup(""));
	}
}
...
void RegisterInstallFunctions() {
    ...
    RegisterFunction("is_16Ginand", Is16GInandFn);/* <--LH_SWRD_CL1_Mervins@2011.07.14--> */
}
