...
//	}
	...
	public TransProtocol(int type, String arg) {
		mCommandType = type;
		mCommandArg = arg;
	}
	
	public TransProtocol(int type) {
		mCommandType = type;
	}
	...
	private void handleQueryStatus(String cmdArg) {
		...
		// 实际使用将替换固定的IP和端口
		client = new TransChannelClient(mConfig.getServerIp(),
				TransConfig.COMMAND_PORT + 1);
	...
	private void handleTransData(String cmdArg) {
		...
		TransChannelServer server = new TransChannelServer(
		...
		if (mListener != null) {
			if (res) {
				...
				int deviceNum = Integer.valueOf(shortName.substring(0, shortName.lastIndexOf(".")).split("-")[3]);
				mConfig.setDeviceNumber(deviceNum);
	...
	public void transData(String srcFile) {
	...
	private void transData(File file) {
		// FIXME: 测试使用端口号 + 1
		TransChannelClient data_client = new TransChannelClient(mConfig.getServerIp(), 
				TransConfig.DATA_PORT);
		if (data_client != null) {
			try {
	...
	private void transDataToNext(final File file) {
		...
		final TransChannelClient command_client = new TransChannelClient(
				mConfig.getServerIp(), TransConfig.COMMAND_PORT);
		long size = file.length();
		String oldName = file.getName();
		String shortName = oldName.substring(0, oldName.lastIndexOf("."));
		Log.e(TAG, "shortName = " + shortName);
		String[] part2 = shortName.split("-");
		String newName = String.format("%s/%s-%s-%s-%d.zip", file.getParent(), part2[0], part2[1], part2[2], Integer.valueOf(part2[3]) + 1);
		String cmdArg = String.format("%s:%d", newName, size);
