...
class VideoSelect(SocketServer.StreamRequestHandler):
	...
	def handle(self):
		...
		if cmds[0].lower() == 'start' and not dic.has_key(self.client_video) and len(dic) < 5:
			...
			cmd = 'gtech -vvv %s --no-audio --quiet --daemon --sout=#transcode{vcodec=h264,vb=500,scale=1}:duplicate{dst=std{access=http,mux=asf,dst=%s:%s}}' %(v[0],localip,str(self.client_port))
                	print cmd
			x = EasyProcess()
