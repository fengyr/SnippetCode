...
"""
	...
	配置表采用如下格式:
	站点:视频编号:组播地址:组播端口:图像格式(mpeg2|mpeg4|h264)
...
class VideoSelect(SocketServer.StreamRequestHandler):
	...
	def handle(self):
		...
		try:
			...
			if len(cmds) != 4 or cmds[0].lower() != 'transform' or cmds[2].lower() != 'to': raise Exception('非法命令')
		...
		cmd = 'gtech -vvv %s --no-audio --daemon --quiet --sout=#transcode{vcodec=h264,vb=500,scale=1}:duplicate{dst=std{access=udp,mux=ts,dst=%s:%s}}' % (v[0], self.client_address[0], self.client_port)
		print cmd
		x = EasyProcess(cmd)
		try:
