...
def function(*arg):
	while True:
		...
		try:
			p = os.popen( "netstat -anp | grep gtech | grep ESTABLISHED" )
			lines = p.readlines()
			p.close()
			dicnew = dic.copy()
			for line in lines:
				lnx = [ln for ln in list(line.split(' ')) if len(ln)>0]
				key = lnx[3].split(':')[1]
				print key
				if dicnew.has_key(key):
					dicnew.pop(key)
			for item in dicnew.keys():
				x = EasyProcess()
				x.stop(dicnew[item][0])
				dic.pop(item)
		except IOError:
			pass
		sleep(30)
...
class VideoSelect(SocketServer.StreamRequestHandler):
	...
	def handle(self):
		...
		if cmds[0].lower() == 'start' and not dic.has_key(self.client_video) and len(dic) < 5:
			...
			cmd = 'gtech -vvv http://%s:%s --no-audio --quiet --daemon --sout=#standard{access=http,mux=ts,dst=%s:%s}}' %(a[0],str(self.client_port),localip,str(self.client_port))
            		print cmd
			x = EasyProcess()
			dic[self.client_video] = [x.start(cmd), 1]
			s = controller.init_socket()
			controller.handle(s, 'start:%s\r' % (self.client_port))
			print dic
		...
		if cmds[0].lower() == 'stop':
			...
			if dic.has_key(self.client_video) and dic[self.client_video][1] == 0:
				print dic
				s = controller.init_socket()
				controller.handle(s, 'stop:%s\r' % (self.clent_port))
