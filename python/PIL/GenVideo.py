...
def ParseArgs():
    ...
    parser.add_option('-i', '--input', dest='input_file', type='string', help='source video file')
    parser.add_option('-o', '--output', dest='output_file', type='string', help='destiny video file',
    ...
    parser.add_option('-f', '--format', dest='format', type='string', help= format_help, default=config.FORMAT)
    parser.add_option('--author', dest='author', type='string', help='the author name', default=config.AUTHOR)
    parser.add_option('-r', '--rate', dest='rate', type='int', help='video fps', default=config.FPS)
    parser.add_option('--operate', dest='action', type='string', help=operate_help, default=config.ACTION)
...
def video2pic():
    convert.video2pic(options.input_file, tempfiles, options.duration, options.rate)

def do_pic():
    marker = picmark.Marker(config.DIR, options.format, options.action, Scale=options.scale, Fps=options.rate)
...
def pic2video():
    try:
        ...
        return
    convert.pic2video(tempfiles, options.output_file, CODEC_FORMAT[options.format][0], 
            CODEC_FORMAT[options.format][1], options.scale, options.rate, options.author)

def videodump():
    if options.scale == config.SCALE:
        options.scale = 'vga'
    convert.videodump(options.input_file, options.output_file, options.scale, options.duration)
